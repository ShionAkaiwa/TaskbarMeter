...
| `改善記録.md` | 実測 → 改善 → リリースの記録と、残っている課題の一覧 |
| `LICENSE` | MIT。免責の文言はここが本体 |
| `.gitattributes` | `.cmd` と同梱 `.txt` を CRLF に固定する |

**配布相手が読む 4 本（README / はじめかた / 説明書 / 仕様書）は役割が違う。
機能を変えたら、該当するものを直すこと。** 各ファイルの末尾に他へのリンクがある。

`改善記録.md` は作業用。**直したことと、残っている課題の番号付き一覧**を持っている。
「次はどれをやるか」はここから選ばせる。1 周終えたら追記すること。
...
10. **パフォーマンスカウンタを張り替えるとき、生きているものを作り直さないこと**
    これらは「前回読んだ時点との差」で値を出す。作り直すと基準が消え、直後に読むと
    差がほぼ 0 の区間を見て 0 を返す。全部作り直していたころ、`Volatile` な GPU 指標が
    **5 秒ごとに 0% に落ちて**いた。`CounterMetric.Refresh()` は消えたものだけ Dispose し、
    増えたものだけ new すること。

11. **段階（`Stage`）は前回の値から引っぱって決めること**
    段階ごとに下地（体の形）が違うので、しきい値に張り付いた負荷をそのまま使うと
    1 秒ごとに体が上下して「震え」に見える。撤去した微アニメーションと同じ現象を、
    振幅を大きくして作り直すことになる。`IconFactory.NextStage()` が 3% の遊びを持っている。
    ゲージの赤も同じ理由で、85% で切り替えず 92% にかけて寄せている。

12. **`Update()` はタイマーからだけ呼ぶこと**
    指標の多くは差分で値を出すので、短い間隔で呼ぶと差が取れずに 0% になる。
    メニューや設定画面からの反映は `Reapply()`（設定を読み直し、直前の値で描き直すだけ）。
    以前はどこからも `Update()` を呼んでいて、**メニューを開いている間ずっと CPU が 0%** に
    見えていた。まばたきの位相（`_tick`）も飛ぶ。
...
**免責は 5 か所に置いてある。** `LICENSE`（MIT）、`README.md` 末尾、`はじめかた.md` の
よくある質問、`dist-template/お読みください.txt`、`release-notes.template.md`。
配布相手が読む順に並べてあるので、増やすときも「実際に読む場所」に置くこと。
...
- 内蔵キャラは負荷で 5 段階に変化（うとうと・ふつう・ごきげん・あせり・限界）＋まばたき。
  **表情だけでなく体の高さと頭の毛が変わる**ので、16px でも輪郭だけで段階が読める
- ゲージは上下 2 行を別々に使って 28 段階。半分の位置に切り欠きがある
...
- スキン切り替え（内蔵キャラを猫・スライムなどに）。`StageSprites` と `FaceRows` を
  組で差し替えられるようにする形が素直（段階ごとに下地と顔の行を持つ構造になっている）
