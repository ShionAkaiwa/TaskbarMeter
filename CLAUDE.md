...
- `BuildNumber()` … 数字。白ドットを置いて `Outline()` で 1 ドットの縁を付ける
  - 字形は 2 組ある。`Glyphs`（5x9）と `NarrowGlyphs`（4x9）で、3 文字以上は細いほう。
    `100` は 5 幅 3 つでは 16px に入らず、字間を詰めると 0 と 0 がくっついて読めなくなる
...
**フォームの部品を座標の数値で置かないこと。** 画面の拡大表示（125% / 150% / 200%）では
文字だけが大きくなるので、手で決めた枠から溢れて重なる。**実際にノート PC で崩れた。**

- 並べ方は `TableLayoutPanel` と `AutoSize` に任せる
- 説明文の `Label` は `AutoSize = true` ＋ `MaximumSize` で折り返す
- 余白は数値ではなく `Font.Height` を基準に決める（`Unit`）
- ウィンドウの幅は `GetPreferredSize` の結果から決める。固定幅にすると名前が切れる
- 描画する `Panel` は `ClientSize` を見て描く。作ったときの寸法を覚えて描くと、
  拡大率が変わったときにはみ出す

`SetupForm` と `SkinPreviewForm` はこの方針で書き直してある。戻さないこと。
...
- **アイコン同士の掛け合い**（暇な指標が、限界の指標のほうを見る「待ちぼうけ」の顔）…
  一度実装したが作者の指示で撤去。診断としては読めるが、勝手に目が動くのが好まれなかった。
  **表情は自分の負荷だけで決めること。**
